...
* Fix - Reset Tracking notice is not displayed after resetting the tracking settings.
* Fix - Credit card payment gateway fees are not removed when another gateway from the same Stripe plugin is selected on the classic checkout page while Optimized Checkout is enabled.
* Fix - Incorrect payment method is displayed on the Pay for Order page after placing an order using the Stripe Credit Card gateway.
